...
public class GetEntryHandler implements
    ...
    public APIGatewayProxyResponseEvent handleRequest(final APIGatewayProxyRequestEvent input,
        ...
        final var foodEntries = this.getFoodEntries(currentUser, input);
        return apiGatewayProxyResponseEventHelper.createResponse(foodEntries);
    }

    private List<EntryModel> getFoodEntries(final UserInfo currentUser, APIGatewayProxyRequestEvent input) {
        final var entries = entryRepo.listByUserEmailAndSortBetweenDates(currentUser.email(),
                input.getQueryStringParameters().get("dateFrom"), input.getQueryStringParameters().get("dateTo"));
